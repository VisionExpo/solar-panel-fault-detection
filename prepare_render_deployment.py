...
def copy_deployment_files(model_path, output_dir):
    ...
    # Copy download_model.py script
    if os.path.exists('download_model.py'):
        print(f"Copying download_model.py to {output_dir}...")
        shutil.copy2('download_model.py', os.path.join(output_dir, 'download_model.py'))
        print(f"Copied download_model.py to {output_dir}")
    else:
        print(f"Warning: download_model.py does not exist")
