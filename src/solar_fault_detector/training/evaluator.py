...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            batch_y_true_list.append(np.argmax(batch_y, axis=1))
            batch_y_pred_list.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(batch_y_true_list)
        y_pred = np.concatenate(batch_y_pred_list)

        del batch_y_true_list
        del batch_y_pred_list
