...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            eval_y_true_list.append(np.argmax(batch_y, axis=1))
            eval_y_pred_list.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(eval_y_true_list)
        y_pred = np.concatenate(eval_y_pred_list)
        del eval_y_true_list
        del eval_y_pred_list
