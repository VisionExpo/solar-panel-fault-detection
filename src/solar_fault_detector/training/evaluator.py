...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            true_vals_arr.append(np.argmax(batch_y, axis=1))
            pred_vals_arr.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(true_vals_arr)
        y_pred = np.concatenate(pred_vals_arr)
