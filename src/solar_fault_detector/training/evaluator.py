...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            _y_true_list.append(np.argmax(batch_y, axis=1))
            _y_pred_list.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(_y_true_list)
        y_pred = np.concatenate(_y_pred_list)
        del _y_true_list, _y_pred_list
