...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            y_true_accumulator.append(np.argmax(batch_y, axis=1))
            y_pred_accumulator.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(y_true_accumulator)
        y_pred = np.concatenate(y_pred_accumulator)
        del y_true_accumulator
        del y_pred_accumulator
