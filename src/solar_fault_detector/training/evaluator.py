...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            _y_true_accum.append(np.argmax(batch_y, axis=1))
            _y_pred_accum.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(_y_true_accum)
        y_pred = np.concatenate(_y_pred_accum)
        del _y_true_accum, _y_pred_accum
