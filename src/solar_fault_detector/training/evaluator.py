...
class Evaluator:
    ...
    def evaluate(
        ...
        """
        true_labels_accumulation_list = []
        pred_labels_accumulation_list = []

        for batch_x, batch_y in dataset:
            preds = model(batch_x, training=False).numpy()
            true_labels_accumulation_list.append(np.argmax(batch_y, axis=1))
            pred_labels_accumulation_list.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(true_labels_accumulation_list)
        y_pred = np.concatenate(pred_labels_accumulation_list)
        del true_labels_accumulation_list
        del pred_labels_accumulation_list
