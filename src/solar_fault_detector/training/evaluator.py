...
class Evaluator:
    ...
    def evaluate(
        ...
        for batch_x, batch_y in dataset:
            ...
            y_true_accum_list.append(np.argmax(batch_y, axis=1))
            y_pred_accum_list.append(np.argmax(preds, axis=1))

        y_true = np.concatenate(y_true_accum_list)
        y_pred = np.concatenate(y_pred_accum_list)
        del y_true_accum_list
        del y_pred_accum_list
